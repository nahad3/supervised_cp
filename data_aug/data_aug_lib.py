...
def GenerateRandomCurves(X, sigma=0.2, knot=4):
    ...
    random_curve_arr = np.asarray([])
    for i in range(0,X.shape[1]):
        cs_i = CubicSpline(xx[:,i], yy[:,i])
        random_curve_arr = np.concatenate((random_curve_arr, cs_i(x_range).reshape(-1,1)),axis = 1) if len(random_curve_arr)  \
            else cs_i(x_range).reshape(-1,1)
    return random_curve_arr
...
def DA_FlipLR(X):
    'assumes time series indices are the first axes.'
    return np.flipud(X)
